...
public class IndexService{
	 ...
	 * @return 
	 */
	public static boolean queryId(String thisId,String id){//分别为自己Id，别人id
		if(!thisId.equals(id)){//不可以搜索自己
			Session session = PersonalSessionStorage.getSessionById(id);//通过别人id搜索那个Id的session
			if(session == null){
				return false;
			}
			//查询搜索的id是否正在一对一对话
